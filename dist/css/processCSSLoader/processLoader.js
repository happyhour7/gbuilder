...
function processLoader(browserify, task) {
    ...
    if (task.compress === true) {
        _gulp = _gulp.pipe((0, _gulpCleanCss2.default)({ compatibility: 'ie8' }, function (details) {
            console.log((0, _util.getLogText)(details.name) + "\t" + _cliColor2.default.yellow((0, _util.getLogText)("Original:" + details.stats.originalSize / 1000 + 'KB')));

            console.log((0, _util.getLogText)("----") + "\t" + _cliColor2.default.yellow((0, _util.getLogText)("Minified:" + details.stats.minifiedSize / 1000 + 'KB')));
        }));
    }
    _gulp.on("error", _util.handleError).pipe(_gulp3.default.dest(_distPath)).on('data', function (file) {
        ...
        console.log((0, _util.getLogText)(task.compress ? "----" : buildName) + "\t" + _cliColor2.default.yellow((0, _util.getLogText)("Build:" + file.contents.length / 1000 + 'KB')));
    });
